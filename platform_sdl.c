...
{
	struct platform_sdl_config *passed_conf = (struct platform_sdl_config *)platform_desc->config_handle;
	...
	if (platform_desc && platform_desc->config_handle 
		...
		conf.init_flags = passed_conf->init_flags;
		if (passed_conf->exec_pre_start_hook) {
			conf.exec_pre_start_hook = passed_conf->exec_pre_start_hook;
		}
		if (passed_conf->exec_post_start_hook) {
			conf.exec_post_start_hook = passed_conf->exec_post_start_hook;
		}
	}
...
{
	struct platform_sdl_window_config *passed_conf = (struct platform_sdl_window_config *)window_desc->config_handle;
